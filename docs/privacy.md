...
Default is provider `ollama` at `http://127.0.0.1:11434` with `allow_remote = false`. A non-loopback Ollama URL without that flag is refused. Before the first remote call the CLI prints: `Remote LLM is on — the prompt for this call may leave this machine.` The corpus file is not attached. `dossier doctor` prints `egress: no` or `egress: yes` so you can see the current choice.
