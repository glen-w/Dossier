...
| `employer` | Walks every folder named in `DOSSIER_EMPLOYER_PATHS` or `[employer] paths`. Text files are stored. Other files are inventory lines only. `~/Documents` itself is ignored. |
