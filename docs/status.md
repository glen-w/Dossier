...
Dossier 0.9 writes a posting-shaped CV or letter from spanned approved cards (`dossier tailor`). 0.8 added thin-user sources and hybrid ask over passage vectors in `evidence.db`. Retrieval still prefers a quote. When full text is empty or thin, a high-cosine neighbor can supply the sentence. When one span is not enough, it can stitch compound questions, hop on seeker headers (`ask.hops`, default 1), and call a local model only inside a call budget. `dossier index` embeds passages with Ollama. `ask.pubs` is off unless you turn it on. Fixture tests are green. A full warehouse prove and live pubs ingest (after collection confirm) are still human steps.
...
- CLI: `ingest`, `extract`, `buffet`, `approve`, `ask`, `brief`, `run`, `span`, `defend`, `gaps`, `packet`, `tailor`, `index`, `prove`, `doctor`, `referees`.
