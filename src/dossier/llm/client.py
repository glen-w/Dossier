"""LLM transport: Ollama (httpx) and optional LiteLLM. Shape copied from Paperful."""
...
class LiteLLMClient:
    ...
    def check_config(self, model: str) -> tuple[bool, str]:
        try:
            ...
            return False, "LiteLLM not installed; pip install 'dossier[llm]'"
    ...
    def complete(self, request: CompletionRequest) -> str:
        try:
            ...
            raise LlmExtraMissingError(
                "LiteLLM is not installed; pip install 'dossier[llm]'"
            ) from None
        validate_llm_api_base(self.api_base)
        kwargs: dict[str, Any] = {
            "model": request.model,
